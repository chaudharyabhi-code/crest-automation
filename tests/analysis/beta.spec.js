...
/**
 * Portfolio Beta Test — API vs locally-recomputed
 *
 * SQL is a verbatim port of:
 *   app/crud/dashboard_crud.py :: get_returns_for_volatility
 *   app/crud/dashboard_crud.py :: get_positions_without_transactions
 *
 * Beta formula (identical to calculation_util.calculate_beta):
 *   Var(R_m)      = (Σ R_m² - (Σ R_m)² / n) / (n - 1)
 *   Cov(R_p, R_m) = (Σ R_p·R_m - Σ R_p·Σ R_m / n) / (n - 1)
 *   β             = Cov / Var
 *
 * Required env: USER_ID
 * Optional env: BENCHMARK_ID (default 9 = BSE 500)
 *               BETA_LOOKBACK_DAYS (default 365)
...
function extractApiBeta(body) {
  const data = body?.data;
  if (!data) return { value: null, raw: null };

  let raw = null;
  if (data.beta != null) raw = data.beta;
  else if (data.Beta != null) raw = data.Beta;
  else {
    const candidates = []
...
test.describe('Analysis Beta Tests', () => {
  ...
  test('Beta - API vs SQL', async ({ apiClient }) => {
    ...
    const { value: apiBeta, raw: apiBetaRaw } = extractApiBeta(keyMetricsRes.body);
    if (apiBeta == null || !isFinite(apiBeta)) {
      throw new Error(`Beta value not available in key-metrics API response (got "${apiBetaRaw ?? 'missing'}")`);
    }

    // Compare at the API's display precision (2dp for Beta) so equal-looking
    // values actually compare as equal.
    const dp = 2;
    const apiRounded = parseFloat(apiBeta.toFixed(dp));
    const sqlRounded = parseFloat(sqlBeta.toFixed(dp));
    const diff = Math.abs(apiRounded - sqlRounded);
    const diffPct = Math.abs(apiRounded) > 0
      ? (diff / Math.abs(apiRounded)) * 100
      : (diff === 0 ? 0 : Infinity);
    const sameSign = (apiRounded === 0 && sqlRounded === 0) || Math.sign(apiRounded) === Math.sign(sqlRounded);
...
`;
    ...
    expect(match, `API beta ${apiBeta.toFixed(2)} vs SQL ${sqlBeta.toFixed(2)} — diff ${diffPct.toFixed(2)}% (threshold ${COMPARISON_THRESHOLD_PCT}%, sign match: ${sameSign})`).toBe(true);
  });
