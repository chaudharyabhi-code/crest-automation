...
/**
 * Portfolio Alpha Test — API vs locally-recomputed
 *
 * Mirrors app/service/analysis_service.py::get_key_metrics:
 *   1. Fetch portfolio + benchmark daily returns (decimals)
 *   2. β  = calculate_beta(portfolio, benchmark)             ← aligned by date
 *   3. Rp = calculate_total_return(portfolio_returns)        ← FULL series, annualized
 *   4. Rm = calculate_total_return(benchmark_returns)        ← FULL series, annualized
 *   5. α  = Rp − Rf − β × (Rm − Rf)
 *
 * Required env: USER_ID
 * Optional env: BENCHMARK_ID (default 9 = BSE 500)
 *               ALPHA_LOOKBACK_DAYS (default 365)
...
async function fetchBenchmarkReturns(benchmarkId, startDate, endDate) {
  const { rows } = await dbClient.query(
    `SELECT to_char(price_date, 'YYYY-MM-DD') AS date,
            daily_return / 100.0 AS daily_return
  ...
  const series = new Map();
  for (const r of rows) {
    const v = parseFloat(r.daily_return);
    if (!isFinite(v)) continue;
    series.set(String(r.date), v);
  }
  return series;
}

function calculateBeta(portfolioMap, benchmarkMap) {
  const R_p = [];
  const R_m = [];
  for (const [date, p] of portfolioMap) {
    if (!benchmarkMap.has(date)) continue;
    const b = benchmarkMap.get(date);
    if (p == null || b == null || !isFinite(p) || !isFinite(b)) continue;
    R_p.push(p);
    R_m.push(b);
  }
  ...
  const variance   = (S_x2 - (S_x * S_x) / n) / (n - 1);
  const covariance = (S_xy - (S_x * S_y) / n) / (n - 1);

  if (variance === 0 || !isFinite(variance) || !isFinite(covariance)) return null;
  const beta = covariance / variance;
  return isFinite(beta) ? beta : null;
}

function calculateTotalReturn(seriesMap) {
  ...
  if (n < 2) return null;
  cumulative -= 1;
  const annualized = Math.pow(1 + cumulative, 365.0 / n) - 1;
  return isFinite(annualized) ? annualized : null;
}

function calculateAlpha(portfolioReturn, benchmarkReturn, beta, riskFreeRate) {
  if (beta == null || !isFinite(beta)) return null;
  if (portfolioReturn == null || !isFinite(portfolioReturn)) return null;
  if (benchmarkReturn == null || !isFinite(benchmarkReturn)) return null;
  if (!isFinite(riskFreeRate)) return null;
  const alpha = portfolioReturn - riskFreeRate - beta * (benchmarkReturn - riskFreeRate);
  return isFinite(alpha) ? alpha : null;
}

function extractApiAlpha(body) {
  const data = body?.data;
  if (!data) return { value: null, raw: null };

  let raw = null;
  if (data.alpha != null) raw = data.alpha;
  else if (data.Alpha != null) raw = data.Alpha;
  else {
    const candidates = []
...
test.describe('Analysis Alpha Tests', () => {
  ...
  test('Alpha - API vs SQL', async ({ apiClient }) => {
    ...
    const [portfolioMap, benchmarkMap, keyMetricsRes] = await Promise.all([
      fetchPortfolioReturns([userId], startStr, endStr, {
        onlyPositionsWithTransactions: ONLY_POSITIONS_WITH_TRANSACTIONS,
      }),
      fetchBenchmarkReturns(BENCHMARK_ID, startStr, endStr),
    ...
    const sqlBeta = calculateBeta(portfolioMap, benchmarkMap);
    if (sqlBeta == null || !isFinite(sqlBeta)) {
      throw new Error('SQL beta calculation returned null/invalid');
    }

    const portfolioTotal = calculateTotalReturn(portfolioMap);
    const benchmarkTotal = calculateTotalReturn(benchmarkMap);
    if (portfolioTotal == null || benchmarkTotal == null) {
      throw new Error('SQL total return calculation returned null/invalid');
    }

    const sqlAlphaDecimal = calculateAlpha(portfolioTotal, benchmarkTotal, sqlBeta, RISK_FREE_RATE);
    if (sqlAlphaDecimal == null || !isFinite(sqlAlphaDecimal)) {
      throw new Error('SQL alpha calculation returned null/invalid');
    }
    const sqlAlphaPct = sqlAlphaDecimal * 100;

    const { value: apiAlphaPct, raw: apiAlphaRaw } = extractApiAlpha(keyMetricsRes.body);
    if (apiAlphaPct == null || !isFinite(apiAlphaPct)) {
      throw new Error(`Alpha value not available in key-metrics API response (got "${apiAlphaRaw ?? 'missing'}")`);
    }

    const diff = Math.abs(apiAlphaPct - sqlAlphaPct);
    const diffPct = Math.abs(apiAlphaPct) > 0
      ? (diff / Math.abs(apiAlphaPct)) * 100
      : (diff === 0 ? 0 : Infinity);
    const sameSign = (apiAlphaPct === 0 && sqlAlphaPct === 0) || Math.sign(apiAlphaPct) === Math.sign(sqlAlphaPct);
    const match = sameSign && diffPct <= COMPARISON_THRESHOLD_PCT;

    let aligned = 0;
    let firstAligned = null, lastAligned = null;
    for (const d of portfolioMap.keys()) {
      if (benchmarkMap.has(d)) {
        aligned++;
        if (!firstAligned) firstAligned = d;
        lastAligned = d;
      }
    }
...
Benchmark Rows     : ${benchmarkMap.size}
Aligned Rows       : ${aligned}
Aligned Span       : ${firstAligned ?? '-'} → ${lastAligned ?? '-'}
SQL β              : ${sqlBeta.toFixed(4)}
Portfolio Total Rt : ${(portfolioTotal * 100).toFixed(2)}%
Benchmark Total Rt : ${(benchmarkTotal * 100).toFixed(2)}%
API Alpha          : ${apiAlphaPct.toFixed(1)}%
SQL Alpha          : ${sqlAlphaPct.toFixed(1)}%
...
`;
    ...
    expect(match, `API alpha ${apiAlphaPct.toFixed(1)}% vs SQL ${sqlAlphaPct.toFixed(1)}% — diff ${diffPct.toFixed(2)}% (threshold ${COMPARISON_THRESHOLD_PCT}%, sign match: ${sameSign})`).toBe(true);
  });
