...
/**
 * Portfolio Sharpe Ratio Test — API vs locally-recomputed
 *
 * Mirrors app/utils/calculation_util.py :: calculate_sharpe_ratio:
 *   mean_daily      = mean(portfolio_returns)
 *   annual_return   = mean_daily × trading_days
 *   daily_std       = std(portfolio_returns, ddof=1)
 *   annual_std      = daily_std × √trading_days
 *   sharpe          = (annual_return − risk_free_rate) / annual_std
 *
 * Required env: USER_ID
 * Optional env: SHARPE_LOOKBACK_DAYS (default 365)
...
function calculateSharpeRatio(seriesMap, opts = {}) {
  ...
  const dailyStd     = sampleStd(values);
  const annualStd    = dailyStd * Math.sqrt(tradingDays);

  if (annualStd === 0 || !isFinite(annualStd)) return null;

  const sharpe = (annualReturn - riskFreeRate) / annualStd;
  return isFinite(sharpe) ? sharpe : null;
}

function extractApiSharpe(body) {
  const data = body?.data;
  if (!data) return { value: null, raw: null };

  let raw = null;
  if (data.sharpe != null) raw = data.sharpe;
  else if (data.sharpe_ratio != null) raw = data.sharpe_ratio;
  else if (data['Sharpe Ratio'] != null) raw = data['Sharpe Ratio'];
  else {
    const candidates = []
...
test.describe('Analysis Sharpe Ratio Tests', () => {
  ...
  test('Sharpe Ratio - API vs SQL', async ({ apiClient }) => {
    ...
    const sqlSharpe = calculateSharpeRatio(portfolioMap, {
      riskFreeRate: RISK_FREE_RATE,
      tradingDays:  TRADING_DAYS_PER_YEAR,
    });
    if (sqlSharpe == null || !isFinite(sqlSharpe)) {
      throw new Error('SQL Sharpe calculation returned null/invalid');
    }

    const { value: apiSharpe, raw: apiSharpeRaw } = extractApiSharpe(keyMetricsRes.body);
    if (apiSharpe == null || !isFinite(apiSharpe)) {
      throw new Error(`Sharpe Ratio not available in key-metrics API response (got "${apiSharpeRaw ?? 'missing'}")`);
    }

    const diff = Math.abs(apiSharpe - sqlSharpe);
    const diffPct = Math.abs(apiSharpe) > 0
      ? (diff / Math.abs(apiSharpe)) * 100
      : (diff === 0 ? 0 : Infinity);
    const sameSign = (apiSharpe === 0 && sqlSharpe === 0) || Math.sign(apiSharpe) === Math.sign(sqlSharpe);
    ...
    const dailyStd  = sampleStd(valuesArr);
    const annualStd = dailyStd * Math.sqrt(TRADING_DAYS_PER_YEAR);
...
Daily Std (ddof=1) : ${isFinite(dailyStd)  ? dailyStd.toExponential(4)  : 'null'}
Annualized Std     : ${isFinite(annualStd) ? annualStd.toFixed(6) : 'null'}
API Sharpe         : ${apiSharpe.toFixed(2)}
SQL Sharpe         : ${sqlSharpe.toFixed(2)}
...
`;
    ...
    expect(match, `API Sharpe ${apiSharpe.toFixed(2)} vs SQL ${sqlSharpe.toFixed(2)} — diff ${diffPct.toFixed(2)}% (threshold ${COMPARISON_THRESHOLD_PCT}%, sign match: ${sameSign})`).toBe(true);
  });
