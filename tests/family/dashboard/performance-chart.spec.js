...
test.describe('Performance Chart Tests (Family)', () => {
  ...
  test('Performance Chart Latest Value Verification', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(
      dashboardEndpoints.performanceChart(familyId, {
        fromDate,
        toDate,
        assetClassId,
        entityType
    ...
    const result = await compareApiWithSql({
      ...
      sqlFilePath: 'family/total_wealth_test.sql', // Using the same SQL for total wealth
      familyId: familyId,
    ...
    test.info().attach('performance-chart-details.json', {
      body: JSON.stringify({
        parameters: { familyId, fromDate, toDate, assetClassId, entityType },
