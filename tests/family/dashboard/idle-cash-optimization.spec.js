...
import { compareApiWithSql } from '../../../utils/family/testHelpers.js';
...
test.describe('Idle Cash Optimization Tests (Family)', () => {
  ...
  test('Idle Cash Optimization Value Verification', async ({ apiClient }) => {
    ...
    const potentialSavingsResponse = await apiClient.get(
      dashboardEndpoints.potentialSavings(familyId)
    );
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'family', 'idle_cash_optimization.sql'),
      'utf-8'
...
Comparison:
  ...
  Result:                    ${comparison.message}`;
    ...
    test.info().attach('idle-cash-optimization-details.json', {
      body: JSON.stringify({
        parameters: { familyId, benchmarkCode, fromDate, range },
        inputs: {
  ...
  test('Idle Cash Percentage of Total Wealth Verification', async ({ apiClient }) => {
    ...
    const totalWealthQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'family', 'total_wealth_test.sql'),
      'utf-8'
    ).replace(/{FAMILY_ID}/g, familyId);
