...
test.describe('Portfolio Volatility Tests (Family)', () => {
  ...
  test('Portfolio Volatility Verification', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(dashboardEndpoints.volatility(familyId));
    ...
    const result = await compareApiWithSql({
      ...
      sqlFilePath: 'family/portfolio_volatility_test.sql', // SQL file name from queries/ folder
      familyId: familyId,
