...
import { validateBenchmarkDate, compareBenchmarkValues } from '../../../utils/family/testHelpers.js';
import { familyAnalysisEndpoints as analysisEndpoints } from '../../../endpoints/index.js';

test.describe('Analysis Benchmark Comparison Tests (Family)', () => {
  ...
  test('Benchmark as_of_date Validation - All dates should be today', async () => {
    ...
    const result = await validateBenchmarkDate({
      sqlFilePath: 'family/benchmark_comparison.sql',
  ...
  test('Benchmark Values Comparison - API vs SQL', async ({ apiClient }) => {
    ...
    // Call the API
    const response = await apiClient.get(analysisEndpoints.benchmarkComparison(familyId));
