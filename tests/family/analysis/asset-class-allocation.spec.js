...
test.describe('Asset Class Allocation Tests (Family)', () => {
  ...
  test('Asset Class Allocation Verification', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(analysisEndpoints.assetClassAllocation(familyId));
    ...
    // 2. Get SQL results
    const sqlFilePath = path.join(process.cwd(), 'queries', 'family', 'asset_class-allocation-analysis.sql');
    let sqlQuery = fs.readFileSync(sqlFilePath, 'utf-8');
    sqlQuery = sqlQuery.replace(/{FAMILY_ID}/g, familyId);
