...
test.describe('Geography Allocation Tests (Family)', () => {
  ...
  test('Geography Allocation Verification', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(analysisEndpoints.geographyAllocation(familyId));
    ...
    // 2. Get SQL results
    const sqlFilePath = path.join(process.cwd(), 'queries', 'family', 'geography-allocation-analysis.sql');
