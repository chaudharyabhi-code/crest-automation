...
import { compareDividendsCount } from '../../../utils/family/testHelpers.js';
import { familyAnalysisEndpoints as analysisEndpoints } from '../../../endpoints/index.js';

test.describe('Analysis Dividends Count Tests (Family)', () => {
  ...
  test('Recent Dividends Count - Last 1 Year', async ({ apiClient }) => {
    ...
    const response = await apiClient.get(analysisEndpoints.recentDividends(familyId));
    ...
    const result = await compareDividendsCount({
      apiCount: apiCount,
      sqlFilePath: 'family/recent_dividends_count.sql',
      familyId: familyId,
