...
test.describe('Manage Assets - Risk Concentration Recurring Deposits / RD (manual)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Recurring Deposits / RD (manual)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Recurring Deposits / RD (manual) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-rd.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-rd-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-rd-manual.json', {
