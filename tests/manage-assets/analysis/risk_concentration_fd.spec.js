...
test.describe('Manage Assets - Risk Concentration Fixed Deposits / FD (manual)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Fixed Deposits / FD (manual)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Fixed Deposits / FD (manual) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-fd.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-fd-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-fd-manual.json', {
