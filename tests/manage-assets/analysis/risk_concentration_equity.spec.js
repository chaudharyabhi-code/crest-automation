...
test.describe('Manage Assets - Risk Concentration Equity (automated + manual)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Equity (automated + manual)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Equity (automated + manual) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-equity.sql'), 'utf-8'
    ...
    console.log(`DB returned ${dbRows.length} contributors (incl. manual assets)`);
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-equity-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-equity-manual.json', {
