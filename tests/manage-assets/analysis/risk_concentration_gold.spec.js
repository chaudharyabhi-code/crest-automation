...
test.describe('Manage Assets - Risk Concentration Gold (with Manual Assets)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Gold (with Manual Assets)', async ({ apiClient }) => {
    const userId = process.env.USER_ID;
    const assetClassId = process.env.ASSET_CLASS_ID_GOLD_RISK || '20';
    const threshold = parseFloat(process.env.COMPARISON_THRESHOLD_PCT || '0.25');

    console.log('\n=== Risk Concentration - Gold (with Manual Assets) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-gold.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-gold-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-gold-manual.json', {
