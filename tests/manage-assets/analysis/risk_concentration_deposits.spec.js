...
test.describe('Manage Assets - Risk Concentration Cash Deposits (automated + manual)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Cash Deposits (automated + manual)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Cash Deposits (automated + manual) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-deposits.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-deposits-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-deposits-manual.json', {
