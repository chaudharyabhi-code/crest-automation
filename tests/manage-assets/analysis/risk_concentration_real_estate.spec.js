...
test.describe('Manage Assets - Risk Concentration Real Estate (with Manual Assets)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Real Estate (with Manual Assets)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Real Estate (with Manual Assets) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-real_estate.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-real-estate-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-real-estate-manual.json', {
