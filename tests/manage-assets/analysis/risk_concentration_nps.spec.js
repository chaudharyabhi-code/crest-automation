...
test.describe('Manage Assets - Risk Concentration NPS (automated + manual)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - NPS (automated + manual)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - NPS (automated + manual) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-nps.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-nps-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-nps-manual.json', {
