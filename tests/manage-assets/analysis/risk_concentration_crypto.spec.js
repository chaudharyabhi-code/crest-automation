...
test.describe('Manage Assets - Risk Concentration Crypto (with Manual Assets)', () => {

  test.beforeAll(async () => { await dbClient.init(); });
  test.afterAll(async () => { await dbClient.close(); });

  test('Risk Concentration - Crypto (with Manual Assets)', async ({ apiClient }) => {
    ...
    console.log('\n=== Risk Concentration - Crypto (with Manual Assets) ===');
    ...
    const sqlQuery = fs.readFileSync(
      path.join(process.cwd(), 'queries', 'manage-assets', 'risk_concentration-crypto.sql'), 'utf-8'
...
=== Comparison ===\n`;
    ...
    test.info().attach('risk-concentration-crypto-manual.txt', { body: report, contentType: 'text/plain' });
    test.info().attach('risk-concentration-crypto-manual.json', {
