...
WITH params AS (
  SELECT
    (SELECT ARRAY_AGG(user_id) FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)::int[]      AS user_ids,        -- <-- replace with real user ids
...
mf_holdings AS (
  ...
  FROM mf ma
  CROSS JOIN params p
  WHERE ma.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
...
mf_latest AS (
  ...
  FROM mf m
  CROSS JOIN params p
  WHERE m.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
