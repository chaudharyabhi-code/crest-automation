...
user_mf AS (
    ...
    FROM mf
    WHERE mf.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true) AND mf.deleted_at IS NULL AND mf.current_value > 0
...
user_etf AS (
    ...
    WHERE eh.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true) AND eh.deleted_at IS NULL AND eh.current_value > 0
...
equity_raw AS (
    ...
    WHERE dh.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true) AND dh.deleted_at IS NULL
...
nps_agg AS (
    SELECT DISTINCT ON (n.user_id)
        ...
        'OTHER' AS asset_type
    FROM nps n
    WHERE n.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true) AND n.deleted_at IS NULL
