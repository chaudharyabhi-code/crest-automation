...
config AS (
    SELECT
        (SELECT ARRAY_AGG(user_id) FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)::int[] AS user_ids,
...
deposits_latest AS (
    ...
    CROSS JOIN config
    WHERE bs.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
...
recurring_latest AS (
    ...
    CROSS JOIN config
    WHERE rdt.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
...
term_latest AS (
    ...
    CROSS JOIN config
    WHERE tdt.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
...
equity_latest AS (
    ...
    CROSS JOIN config
    WHERE el.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
...
mf_latest AS (
    ...
    FROM mf_ledger mfl
    CROSS JOIN config
    WHERE mfl.user_id IN (SELECT user_id FROM family_members WHERE family_id = {FAMILY_ID} AND is_invitation_accepted = true)
