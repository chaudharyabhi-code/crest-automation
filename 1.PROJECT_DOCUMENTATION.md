...
#### Issue 1: "Database query error: syntax error"
**Cause**: SQL placeholders not replaced correctly
**Solution**:
- Check if `{USER_ID}` is in SQL
- Ensure `userId` is passed to `compareApiWithSql()`
- For historical queries, add `endDate` parameter

#### Issue 2: "Total wealth value not found in API response"
**Cause**: API response structure doesn't match extraction logic
**Solution**:
- Log the full response: `console.log(JSON.stringify(response.body, null, 2))`
- Check actual structure
- Update extraction logic to match actual response

#### Issue 3: "Comparison failed with X% difference"
**Cause**: API and DB values don't match within threshold
**Solution**:
- Check if units are converted correctly
- Verify SQL query is calculating correctly
- Check if historical date filtering is working in SQL
- Review API filters (asset_class_id, dates)

#### Issue 4: "Holdings count mismatch"
**Cause**: API and DB counting different things
**Solution**:
- Verify SQL counts individual holdings (not accounts)
- Check if SQL filter is applied correctly
- Ensure both API and SQL use same filter logic

---

## Testing Checklist

### Before Adding New Test Case

- [ ] API endpoint exists and is accessible
- [ ] Database tables/columns exist
- [ ] SQL query tested manually and returns correct data
- [ ] Environment variables configured
- [ ] User has data for the test scenario

### After Creating Test Case

- [ ] Test runs successfully
- [ ] Test passes with expected data
- [ ] Test fails appropriately with invalid data
- [ ] Report is attached and readable
...
- [ ] SQL files have descriptive comments
...
### What We Have
- ✅ **143 automated tests** covering critical scenarios
- ✅ **10 test files** organized by feature
- ✅ **2 test folders** (dashboard, analysis)
- ✅ **Flexible configuration** via environment variables
- ✅ **Comprehensive validation** with threshold and exact match tests

### How to Work with It
1. **Configure** your `.env` file with proper values
2. **Run tests** regularly to catch issues
3. **Add new tests** following the patterns above
4. **Review reports** when tests fail
5. **Maintain SQL** queries as database evolves

### Key Principles
- **DRY (Don't Repeat Yourself)**: Reuse helper functions
- **Single Responsibility**: Each function has one clear purpose
- **Configuration over Code**: Use .env for all settings
- **Clear Reporting**: Every test generates readable reports
...
For questions about:
- **Test failures**: Check HTML report and console logs
- **Database issues**: Verify connection settings in .env
- **API changes**: Update endpoint files accordingly
- **New requirements**: Follow "How to Add New Test Cases" guide

**All 143 tests are production-ready and fully documented!** ✅
